...
**Matrix id:** `java-junit5-rest_assured-selenide`  
**Module id:** `tests-java-junit5-rest_assured-selenide`  
**Catalog:** [autotests-ai/java-junit5-rest_assured-selenide](https://github.com/autotests-ai/java-junit5-rest_assured-selenide) — org showcase / template, **not** a student homework repo.  
**Source:** living sibling [autotests-ai-multistack-app](https://github.com/autotests-ai/autotests-ai-multistack-app) `tests/java/tests-java-junit5-rest_assured-selenide` (bootstrap copy, not Jinja2).

## Verify

HQ `stack-verify` (Phase A: `path_safe` / README / `.gitignore`) is required before publish. compile / smoke / Allure / GHA runners are planned gates — not claimed here.

```bash
python scripts/stack-verify.py --root generated-projects/java-junit5-rest_assured-selenide --profile java-junit5-rest_assured-selenide
```

## GitHub / Allure

- This tree is the catalog cell (`github.com/autotests-ai/java-junit5-rest_assured-selenide`).
- Allure 3 pins: `package.json` + `allurerc.mjs` + `package-lock.json`.
- Living CI orchestrator stays in the clone [`.github/workflows/ci.yml`](https://github.com/autotests-ai/autotests-ai-multistack-app/blob/main/.github/workflows/ci.yml). This repo keeps composite actions under `.github/actions/`.

Canonical Java automation module.
...
One task `test`; the layer is a tag filter, the stand is `-Denv` ([pyramid-map.yaml](https://github.com/autotests-ai/autotests-ai-multistack-app/blob/main/_contract/pyramid-map.yaml)).
